    ...
    for (let gx = 0; gx < gridW; gx++) {
      ...
      for (let y = y0; y < y1; y++) {
        for (let x = x0; x < x1; x++) {
          ...
          const dr = data[o] - bgColor[0];
          const dg = data[o + 1] - bgColor[1];
          const db = data[o + 2] - bgColor[2];
          if (Math.sqrt(dr * dr + dg * dg + db * db) < bgThreshold) bgCount++;
        }
      }
...
function detectContentBoundingBox(srcImageData, options = {}) {
  const { bgColor = [0, 0, 0], bgThreshold = 12, useAlpha = true, padding = 0.04 } = options;
  const { data, width, height } = srcImageData;
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (useAlpha && data[o + 3] < 128) {
        ...
        const dg = data[o + 1] - bgColor[1];
        const db = data[o + 2] - bgColor[2];
        isBg = Math.sqrt(dr * dr + dg * dg + db * db) < bgThreshold;
      }
      if (!isBg) {
        if (x < minX) minX = x;
        if (x > maxX) maxX = x;
